...
class TableToText:
    ...
    def train(self):
        data_loaders = self.get_data(splits=("train", "validation", "test"))
        ...
        lr_scheduler = get_linear_schedule_with_warmup(
            ...
            num_warmup_steps=0,
            num_training_steps=(
                (len(data_loaders["train"]) + len(data_loaders["validation"]) + len(data_loaders["test"])) 
                * self.epochs),
        )
    ...
    def predict(self, model, src, num_sequences):
...
if __name__ == '__main__':
    ...
    argparser.add_argument("-r", "--lr", dest="lr", type=float, default=5e-3,
