...
class TransformerModel(nn.Module):
    ...
    # def forward(self, sentence_input):
        ...
        # return tag_scores
    def forward(self, sentence_input):
        distilbert_output = self.encoder(sentence_input)
        # Adding encoding of  the last and the first layer
        encoded=distilbert_output.last_hidden_state + distilbert_output.hidden_states[0]
        tag_space = self.classification_head(encoded)
        tag_scores = F.log_softmax(tag_space, dim=-1)
        # TODO modify the tag_scores to use the parameters of the crf_layer
        return tag_scores
...
class FinetuneTagger:
    ...
    def add(char):
        return FinetuneTagger.random_alphabet + char
    
    def sub(char):
        return FinetuneTagger.random_alphabet

    def load_training_data(self, trainfile):

        # modify noise list
        noise_functions = {
            "add": FinetuneTagger.add,
            "drop": lambda char: "",
            "sub": FinetuneTagger.sub
        }
        ...
        for sent, tags in list(self.training_data):
            noise = []
            for word in sent:
                random_nosie = ""
                for char in word:
                    if random.random() < 0.2:
                        random_func = random.choice(list(noise_functions.keys()))
                        random_nosie += noise_functions[random_func](char)
                    else:
                        random_nosie += char
                noise.append(random_nosie)
            self.training_data.append((noise, tags))
